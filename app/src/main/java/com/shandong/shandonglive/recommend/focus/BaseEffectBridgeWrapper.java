...
public class BaseEffectBridgeWrapper extends BaseEffectBridge {
	...
	public void onFocusViewRoot(View focusView, float scaleX, float scaleY, View rootView) {
		if (focusView != null) {
			focusView.animate().scaleX(scaleX).scaleY(scaleY).setDuration(DEFAULT_TRAN_DUR_ANIM).start(); // 放大焦点VIEW的动画.
			runTranslateAnimationRoot(focusView, scaleX, scaleY,rootView); // 移动边框的动画。
		}
	}
	...
	public Rect findLocationWithView(View view, View rootView) {
	...
	public void runTranslateAnimationRoot(View toView, float scaleX, float scaleY, View rootView) {
		try {
			flyWhiteBorderRoot(toView, getMainUpView(), scaleX, scaleY,rootView);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
